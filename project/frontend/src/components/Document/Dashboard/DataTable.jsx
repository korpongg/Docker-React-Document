...
>
  ...
  >
    ...
    {/* ดำเนินการ */}
    {isAdmin && Number(row.reply) !== 1 && (
      <Tooltip title="ดำเนินการ" arrow>
        <IconButton
          size="small"
          onClick={() => {
            onProcess(row);
    ...
    {/* แก้ไขการดำเนินการ */}
    {isAdmin && Number(row.reply) === 1 && (
      <Tooltip title="แก้ไขการดำเนินการ" arrow>
        <IconButton
          size="small"
          onClick={() => {
            onEditProcess(row);
  ...
  const filteredRows = useMemo(() => {
    return data.filter((row) => {
      ...
      const start =
        !startDate || new Date(row.date_document) >= new Date(startDate);

      const end = !endDate || new Date(row.date_document) <= new Date(endDate);

      const status = !statusFilter || String(row.reply) === statusFilter;
      ...
      <Tooltip title={copied ? "คัดลอกแล้ว!" : "กดเพื่อคัดลอก"} arrow>
        <Chip
          ...
          sx={{
            ...
            border: copied ? "1px solid #22c55e" : "1px solid #c7d2fe",
...
/>
      ...
      },
      {
        ...
        align: "center",
        headerAlign: "center",
        renderCell: (params) => <DocumentChip value={params.value} />,
      },
      {
        field: "date_document",
...
========================= */
            ...
            >
              <Chip
                ...
                sx={{
                  ...
                  boxShadow: done ? "0 4px 10px rgba(15,118,110,0.25)" : "none",
  ...
  userData={userData}
  onEditRequest={(row) =>
    handleEditClick2(row.id, row)
  }
  onProcess={(row) =>
    handleViewClick(row.id, row)
  }
  onEditProcess={(row) =>
    handleEditClick4(row.id, row)
  }
  onDelete={(row) =>
    handleDeleteClick(row.id, row)
  }
/>
        ),
      },
