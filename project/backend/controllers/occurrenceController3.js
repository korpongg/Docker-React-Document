...
const Occurrences = require("../models/Occurrences3");
...
exports.getAllOccurrences = async (req, res) => {
  try {
    ...
    const occQuery = `
      SELECT occ.*
        FROM ${DB_NAME}.[dbo].[occurrences3] occ
    ...
    if (occurrence.acceptby === null && req.body.renew) {
      ...
      if (resAccept) {
        ...
        const emailMessage = `<div style="font-size: 18px;">
          <p style="font-size: 20px;">เรียนผู้บริหารรับทราบ ข้อร้องเรียนระดับความเสี่ยงสูง</p><br/>
